...
| 操作 | 需要重建什么 | 原因 |
|------|-------------|------|
| 修改 `EMBEDDING_MODEL` 或 `EMBEDDING_PROVIDER` | **删除** `storage/knowledge/vector/chroma/` + **清空 `storage/memory/chroma/`** **再** rebuild | 新模型的向量维度可能不同，旧索引/旧向量的维度与新模型不兼容 |
...
- [ ] 清空 `storage/memory/chroma/`（长期记忆向量，维度会变）
