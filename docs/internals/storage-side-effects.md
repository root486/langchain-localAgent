...
| 保存路径 | 触发的副作用 | 是否阻塞 |
|----------|-------------|---------|
| `memory/MEMORY.md` | `memory_indexer.rebuild_index()` | 是 |
...
- 修改 `memory/MEMORY.md` 的 rebuild 是同步的，大文件时注意超时风险
...
| `EMBEDDING_MODEL` | 是 | **是** (vector + memory_index) | 是 | 向量维度可能变化 |
| `EMBEDDING_PROVIDER` | 是 | **是** (vector + memory_index) | 是 | 不同 provider 的模型不同 |
